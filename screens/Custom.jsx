...
export default function Custom(props) {
    ...
    const handlePressGroup2 = (option) => {
        ...
        setSelectedOptionRim(option.label)
    };
    ...
    return (
        <ScrollView style={{ backgroundColor: 'rgb(237, 237, 237)', height: '100%', width: '100%' }}>
                    ...
                    <View >
                        ...
                        {loaded5 ?
                            <View style={{ display: 'flex', flexDirection: 'column' }}>
                                <Image source={{ uri: photoVehicle }} alt="make it" style={{ width: 400, height: 200 }} />

                                <Text style={{ backgroundColor: 'black', width: 420, height: 30, color: 'white', padding: 5 }}>COLOR SELECTION</Text>
                                    ...
                                    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', flexDirection: 'row', width: 400,height:150 }}>
                                        {optionsGroup1.map((option) => (
                                            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                                                ...
                                                <Image source={{ uri: colors.find(color => color._id === option.id)?.color_code }} alt={colors[0]?.name} style={{ width: 120, height: 120, marginTop: -30 , position:'absolute',marginLeft:-25}}  />
                                ...
                                <Text style={{ backgroundColor: 'black', width: 420, height: 30, color: 'white', padding: 5, textAlign: 'right' , marginTop:50}}>RIMS SELECTION</Text>
            ...
            <View>
                <Text style={{ backgroundColor: 'black', width: 420, height: 30, color: 'white', padding: 5, textAlign: 'center',fontSize:15 }} >ADD TO RESUME</Text>
                <View style={{ flexDirection: 'column', justifyContent: 'flex-end' }} >
                    <View>
                        <Image source={photoVehicle} alt="Imagen de ejemplo"/>
                        <Text style={{ backgroundColor: 'grey', width: 420, height: 50, color: 'white', textAlign: 'center' ,fontSize:20,padding:8}}>{car?.name}</Text>
                        <Text style={{ backgroundColor: 'black', width: 420, height: 30, color: 'white', padding: 5, textAlign: 'center',fontSize:15 }}>${(car?.price)?.toLocaleString("es-VE")}</Text>
                        <Text style={{ backgroundColor: 'grey', width: 420, height: 50, color: 'white', textAlign: 'center',fontSize:20 ,padding:8}}>{colors[parseInt(selectedOption.charAt(selectedOption.length - 1)) - 1]?.name}</Text>
                        <Text style={{ backgroundColor: 'black', width: 420, height: 30, color: 'white', padding: 5, textAlign: 'center',fontSize:15 }}>${(colors[parseInt(selectedOption.charAt(selectedOption.length - 1)) - 1]?.price_color)?.toLocaleString("es-VE")}</Text>
                        <Text style={{ backgroundColor: 'grey', width: 420, height: 50, color: 'white', textAlign: 'center',fontSize:20 }}>{rims[parseInt(selectedOptionRim.charAt(selectedOptionRim.length - 1)) - 1]?.name}</Text>
                        <Text style={{ backgroundColor: 'black', width: 420, height: 30, color: 'white', padding: 5, textAlign: 'center',fontSize:15 }}>${(rims[parseInt(selectedOptionRim.charAt(selectedOptionRim.length - 1)) - 1]?.price_rim)?.toLocaleString("es-VE")}</Text>
                        <Text style={{ backgroundColor: 'grey', width: 420, height: 50, color: 'white', textAlign: 'center',fontSize:20,padding:8 }}>TOTAL</Text>
                        <Text style={{ backgroundColor: 'black', width: 420, height: 30, color: 'white', padding: 5, textAlign: 'center',fontSize:15 }}>${(car?.price + colors[parseInt(selectedOption.charAt(selectedOption.length - 1)) - 1]?.price_color + rims[parseInt(selectedOptionRim.charAt(selectedOptionRim.length - 1)) - 1]?.price_rim)?.toLocaleString("es-VE")}</Text>
